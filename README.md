...
Apple Silicon (M-series), single thread. Corpus: synthetic 200k-row xlsx
(6.75 MiB zip → 46 MiB XML, 1.2M cells), generated by `tools/gen_corpus.py`.
Identical workload for every contender: inflate, parse, resolve shared
strings, emit gap-preserving quoted CSV (outputs verified character-identical).

| Implementation | best time | vs nanoxml |
|---|---|---|
| **nanoxml** (Zig 0.16, ReleaseFast) | **179 ms** | 1× |
| Microsoft `DocumentFormat.OpenXml` 3.3.0, `OpenXmlReader` streaming (.NET 10, Release, post-JIT best) | 833 ms | 4.7× slower |
| Python stdlib (`zipfile` + C `ElementTree` + `csv`) | 2258 ms | 12.6× slower |

Run it yourself: `nanoxml bench corpus/big.xlsx 10 full` and
`tools/interop/bin/Release/net10.0/interop bench corpus/big.xlsx 5`.

nanoxml stage breakdown:
